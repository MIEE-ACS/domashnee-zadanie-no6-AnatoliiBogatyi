...
namespace DZ6
{
    class Euro : Currency
    {
        ...
        private static decimal toRubExchangeRate = 90;
        ...
        private static readonly List<Euro> bills = new List<Euro>();
        internal Euro(decimal amount)
        {
            Amount = amount;
            bills.Add(this);
        }

        public override bool Equals(object obj) => (obj is Euro) && GetHashCode().Equals(obj.GetHashCode());

        public static bool operator ==(Euro obj1, object obj2) => obj1.Equals(obj2);
        ...
        public override string ToString() =>
                "==========================\nAmount (EUR):\t" + amount.ToString() +
                "\nAmount (RUB):\t" + ToRub().ToString() +
                "\n==========================\n\n";

        internal static string ToList()
        {
            StringBuilder sb = new StringBuilder("LIST OF 'EURO' OBJECTS\n\n");
            foreach (var item in bills)
                sb.Append(item.ToString());
            return sb.ToString();
        }

        internal static void Destroy(ref Euro bill)
        {
